...
in
assert lib.assertMsg (builtins.tryEval (
  architectures.validate "x86_64-linux"
)).success "x86_64-linux must be a supported architecture";
assert lib.assertMsg (builtins.tryEval (
  architectures.validate "aarch64-linux"
)).success "aarch64-linux must be a supported architecture";
assert lib.assertMsg (!(builtins.tryEval (
  architectures.validate "riscv64-linux"
)).success) "unsupported architectures must be rejected";
...
assert lib.assertMsg (profiles.validate profiles.available == profiles.available)
  "all discovered profiles must validate";
assert lib.assertMsg (!(builtins.tryEval (
  profiles.validate [ "does-not-exist" ]
)).success) "unknown profiles must be rejected";
...
assert lib.assertMsg (roles.validate expectedRoles == expectedRoles)
  "all expected roles must validate";
assert lib.assertMsg (!(builtins.tryEval (
  roles.validate [ "does-not-exist" ]
)).success) "unknown roles must be rejected";
