...
let
  ...
  expectedRoles = [ "ci" "workstation" ];
in
assert succeeds (architectures.validate "x86_64-linux")
  "x86_64-linux must be a supported architecture";
assert succeeds (architectures.validate "aarch64-linux")
...
assert builtins.elem "machine" hosts.available
  "machine must be discovered as a host";
assert builtins.elem "ci" hosts.available
  "ci must be discovered as a host";
assert hosts.default == "machine"
  "machine must remain the explicit production host";
assert hosts.definitions.ci.machine.architectures == [ "x86_64-linux" "aarch64-linux" ]
